...
export function SearchBar1({
  ...
  const [local, setLocal] = useState<string>(new URLSearchParams(window.location.search).get('local') || '');
  const [category, setCategory] = useState<string>(new URLSearchParams(window.location.search).get('categories') || '');
